...
namespace GomoryFirstMethodTeaching
{
    public class GomoryFirstTeaching : AbstractDynamicModel
    {
        private const string DescriptionLabel = "Перший алгоритм Гоморі";

        private const string InitialProblemEnteringLabel = "Введіть задачу дискретного програмування:";

        private const string IsNotSuitableProblemLabel = "Дану задачу не можна розв’язати першим алгоритмом Гоморі" +
                                                         "(вона не повністю цілочисельна)";
        ...
        private readonly GomoryFirst _gomoryFirst = new GomoryFirst();
        ...
        {
            var nextAction = _gomoryFirst.IsWeakenedProblemNormalized(InitialProblem)
                               ? SolveWeakenedLabel
        ...
        private UiDescription FirstWeakenedProblemNormalizingStep(UiDescription userData)
        {
            ...
            _gomoryFirst.SolveInitialWeekenedProblem(InitialProblem, out _weakenedNormalizedProblem, out _currentSimplexTable);
        ...
        {
            var nextAction = _gomoryFirst.IsEnd((LppResult)userData["result"].Value, InitialProblem)
                                 ? RightResulLabel
                                 : NeedCutoffLabel;
        ...
        {
            var initialProblemResult = _gomoryFirst.GetInitialProblemResult(_currentSimplexTable, _weakenedNormalizedProblem);
        ...
        private UiDescription CutoffMakingStep(UiDescription userData)
        {
            ...
            var creativeVariable = _gomoryFirst.GetCreativeVariable(_currentSimplexTable, InitialProblem);
            ...
            var cutoff = _gomoryFirst.MakeCutoff(_currentSimplexTable, creativeVariable, InitialProblem);
